    ...
    if($id_venta == '' || $token == ''){
        ...
        if($token == $token_tmp){
            ...
            if($sql->fetchColumn() > 0){
                ...
                $sql=$pdo->prepare("SELECT e.ID_Estatus, e.NombreEstatus, v.ID_Venta, v.ID_Tienda, v.Comentario, v.ID_Usuario, v.Fecha, v.Total, u.Nombre 
                    FROM tblestatuspedidos e, tblventas v, tblusuarios u 
                ...
                $sql=$pdo->prepare("SELECT * FROM `tblestatuspedidos`;");
                $sql->execute();
                $estatusPedido = $sql->fetchAll(PDO::FETCH_ASSOC);
                //print_r($pedidos);
